...
export const fetchLayouts = async (courseId: string): Promise<LayoutsInterface[]> => {
...
export const fetchMarkers = async (courselayoutId: string): Promise<MarkersInterface[]> => {
  const res = await fetch(`${API_URL}/markers/${courselayoutId}`);
  if (!res.ok) 
